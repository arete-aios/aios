...
**Everything the system knows, in files the owner owns, loaded only when relevant.**

Memory is the layer people mean when they say second brain. It is one layer of five, and it is the one that decides whether the AI starts every conversation from zero.

The defining property is not size, it is that memory **does not load on every message**. That is what lets it be long. The constitution is expensive and short. Memory is cheap and deep, and it is opened when a topic comes up.
...
- **The owner's areas of life or work**, one folder each, named in their own words rather than in someone else's taxonomy.
- **People.** One row, and later one file, per named human. What was said, quoted rather than summarised.
- **Decisions and their reasons.** Not the outcome alone. The reasoning is what is worth reading in a year.
...
**The test is a new note.** Write one, and see whether it has an obvious home without anyone having to ask. If the answer is a discussion, the structure does not fit this owner yet.
...
**`braindump7`** produces that raw material, and `assets7` produces the inventory half of it.
